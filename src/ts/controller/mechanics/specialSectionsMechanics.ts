...
class Book9sect91 {
    ...
    public static dropHalfBackpack() {

        try {
            // Check if this has been executed
            let coinState : string = state.sectionStates.otherStates['book8sect91'];
            if( coinState ) {
                // Already executed
            ...
            // Get the two lists
            let heads = [] , tails = [];
            let halfIndex = state.actionChart.backpackItems.length / 2;
            ...
            // Store state
            state.sectionStates.otherStates['book8sect91'] = coinState;
    ...
    private static setUI(coinState : string) {
